        ...
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-slate-500">
              ...
              Their profile will appear here after registration. You can then assign their department and specialization.
            </p>
          </CardContent>
        </Card>
      )}
                ...
                <div key={d.id} className="flex items-center justify-between p-4 rounded-lg border bg-slate-50 dark:bg-slate-900 dark:border-slate-800">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center font-bold text-sm">
                      {(d.users?.full_name || 'D').charAt(0)}
                    </div>
                    <div>
                      <p className="font-semibold">Dr. {d.users?.full_name}</p>
                      <p className="text-sm text-slate-500">{d.specialization} · {d.departments?.name ?? 'No dept'}</p>
